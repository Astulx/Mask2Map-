# Single-stage joint training configuration for Mask2Map.
#
# Unlike the two-phase workflow (1Phase seg pre-training → 2Phase full
# training), this config trains both the segmentation head (IMPNet) and the
# vectorization head (MMPNet) simultaneously from scratch in a single run.
#
# Key differences from the two-phase configs:
#   - No `load_from` dependency on a Phase-1 checkpoint.
#   - All losses (segmentation + vectorization) are active from epoch 1.
#   - Training schedule is 24 epochs (equivalent total compute to 12+12).
#   - Backbone still initialises from the same ImageNet/SparseBEV pretrain.
#
# Recommended launch command (8 GPUs):
#   ./tools/dist_train.sh \
#       projects/configs/mask2map/M2M_nusc_r50_full_joint_24ep.py 8
#
# Expected convergence note:
#   Joint training from scratch typically converges to slightly lower mAP
#   than the two-phase approach for the same total epoch budget.  Consider
#   increasing total_epochs (e.g. 48) or using a lower vectorization loss
#   weight (loss_pts / loss_cls) for the first few epochs if instability is
#   observed.
...
model = dict(
    ...
    # Backbone pretrain only – no Phase-1 segmentation checkpoint required.
    ...
    pts_bbox_head=dict(
        ...
        # Vectorization losses (active from the first epoch in joint training)
